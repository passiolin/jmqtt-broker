...
| 稳态连接数 | 指令TPS | PUBACK 确认率 | PUBACK RTT P50/P99 | 状态 |
|---|---|---|---|---|
| 100,000 | 1,438/s | 100% | 0.9 / 2,550 ms | ✅ 无压力 |
| 200,000 | 3,100/s | 100% | 1.7 / 6,443 ms | ✅ 稳定 |
| 300,000(直压) | 5,026/s | 99.9997% | 0.8 / 227.5 ms | ✅ 上行无损 |

- 上行全程无损:累计 438 万条指令,丢失 0 条,重复 0 条,确认率 99.9997%
- 30 万连接下 PUBACK RTT P50 = 0.8ms,入站路径在 30 万存量下非常健康
...
业务闭环(指令+回显,持久会话,256B,QoS1),broker VM 实测(整 VM 含系统进程):

| 连接数 | 单节点 8C CPU 均值/峰值 | 集群 2×4C CPU 均值(每节点) | 单节点内存 | 上行丢失 | 回显送达率 |
|---|---|---|---|---|---|
| 100,000 | 35.7% / 60.7% | 62.5% / 57.9% | 6.9G | 0 | ✅ 100% |
| 200,000 | 49.5% / 63.5% | 82.2% / 85.4% | 8.3G | 0 | ✅ 100% |
| 300,000(直压) | — | — | — | 0 | — |
| 300,000(阶梯) | 55.3% / 60.4% | 10.3%*(窗口含爬坡) | 9.1G | 0 | ❌ 回显积压 |

纯连接(仅 keepalive,零业务消息):

| 连接数 | 单节点 8C CPU 均值/峰值 | 单节点内存 |
|---|---|---|
| 100,000 | 9.6% / 34.2% | 3.2G |
| 200,000 | 19.1% / 50.5% | 5.6G |
| 300,000 | 10.3%* / 15.7% | 8.0G |
| **420,000** | **28.3% / 52.9%** | **9.3G** |
...
| 指标 | 数值 |
|---|---|
| 每 vCPU 承载连接数(业务闭环) | **≈ 3.5 万**(保守 3 万) |
| 每 vCPU 指令 TPS(闭环,含等量回显+心跳) | **≈ 600**(保守) |
| 每 vCPU 承载连接数(纯连接) | **≈ 13 万** |
| 每 1 万连接 CPU 成本(业务闭环) | ≈ 0.3 vCPU |
| 每 1 万连接内存成本 | ≈ 23KB × 1 万 = 230MB(broker)+ 0.4MB(Redis) |
| 上行纯吞吐参考(无回显,10k 连接) | ≈ 15k msg/s |

#### 集群模式开销

单节点没有集群消息面开销。集群模式下每条消息的额外路径:

| 开销来源 | 说明 | 影响 |
|---|---|---|
| **集群总线广播** | 发布节点写入 Kafka `cluster-topic`, 每个节点以独立 group.id 消费全量消息再本地匹配路由。N 节点 = 每条消息被 N 个节点各消费一遍 | 每条消息多一次 Kafka 序列化/反序列化 + 本地路由匹配 |
| **下行双消费** | 后台下发消息写入 `downlink-topic`, 每个节点消费全量、只投递给本地订阅者 | 非持有节点的消费是空转, 浪费 IO |
| **Redis 争用** | 多节点共享同一 Redis(会话/订阅/在途镜像/离线队列), 写入量随节点数线性增长 | Redis 单线程成为瓶颈(压测实测单条 EVAL 延迟从 5ms 涨到 17ms) |

**集群模式的本质代价**:总 vCPU 不变,但每条消息的投递路径多了 Kafka 往返 + 广播扇出 + 本地匹配,
**有效 CPU 占比下降约 20%**(实测闭环 TPS/vCPU 从 600 降至 480)。换来的收益是水平扩展连接数与高可用。

| 指标 | 单节点 8C16G | 集群 2×4C8G |
|---|---|---|
| 闭环 TPS/vCPU | ≈600 | ≈480(~20% 下降, 集群广播开销) |
| 30 万连接 + 5k TPS | 上行 ✅ / 下行 ❌ | 上行 ✅ / 下行 ❌ |

**选型建议**:连接数 ≤30 万、指令 ≤5k TPS 时单节点足够(8C16G 只用到 78% CPU/84% 内存);
超过 30 万或需要高可用时再启用集群。集群的下行 worker 池已并行化(按分区 worker),
后续优化方向是接入路径 Redis 操作合并(当前每接入 5-6 个操作)与下行投递路径 profile。
